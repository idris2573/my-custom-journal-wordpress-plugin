		...
		public function reset_cart_item_link( $link, $cart_item, $cart_item_key ) {
			...
			if ( isset($cart_item['fpd_data']) && $cart_item['fpd_data'] ) {
				...
				if( !empty($url) ) {
					...
					$link = sprintf( '<a href="%s">%s<br /><i style="opacity: 1; font-size: 0.9em;">%s</i></a>', $url, $cart_item['data']->get_title(), FPD_Settings_Labels::get_translation( 'woocommerce', 'cart:_re-edit product' ) );
